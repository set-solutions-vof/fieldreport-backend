...
from src.models.reports.report import ReportDetailSection, ReportEvidenceItem
...
async def test_load_report_detail_sections_fetches_and_maps() -> None:
    ...
    expected_evidence_items = [ReportEvidenceItem.model_construct()]
    ...
    with (
        ...
        patch.object(
            ...
            return_value=(expected_sections, expected_evidence_items),
        ) as map_sections,
    ):
        sections, evidence_items = await reports_service.load_report_detail_sections(report_id)

    assert sections is expected_sections
    assert evidence_items is expected_evidence_items
