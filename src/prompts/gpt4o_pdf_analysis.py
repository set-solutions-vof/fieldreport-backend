...
- key_value_table: sections where labeled fields are paired with values
  (e.g. "Datum: 12-01-2024", "Onderzoeker: Jan de Vries")
- measurement_table: tabular data with multiple rows of numeric or structured measurements
...
- "table_patterns": list of all field labels and column headers found in key_value and
  measurement sections
- "photo_expectations": list of captions or labels found near photo areas

For "likely_sections": only list PRIMARY top-level section headings. Do not include
sub-headings that appear nested within a section (e.g. bold sub-titles within a Conclusie
or Resultaten block).
""".strip()
